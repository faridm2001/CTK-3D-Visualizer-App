...
import brep_file_handler as brep_files_handler
...
class GUI1(ctk.CTk):
	...
	def __create_canvas(self):
		...
		self._canvas = ctk.CTkCanvas(self, bg=self._canvas_color.get())
	...
	def __create_zoom_slider(self):
		ctk.CTkLabel(self, text="Zoom:").place(relx=self.COMMON_X, rely=0.052, relheight=0.035, relwidth=0.1, anchor="ne")

		self._zoom_slider = ctk.CTkSlider(self, from_=1000.0, to=1.1, orientation="horizontal", command=self.__changed)
	...
	def __create_x_rot_slider(self):
		...
		ctk.CTkCheckBox(self, text="", variable=self._check_x_continuos, command=self.__continuous_x).place(relx=self.COMMON_X - 0.1, rely=0.121)
		
		ctk.CTkLabel(self, text="X Rotation:").place(relx=self.COMMON_X, rely=0.123, relheight=0.035, relwidth=0.075, anchor="ne")
		
		self.x_rotation_slider = ctk.CTkSlider(self, from_=-math.pi, to=math.pi, orientation="horizontal", command=self.__changed)
	...
	def __create_y_rot_slider(self):
		...
		ctk.CTkCheckBox(self, text="", variable=self._check_y_continuos, command=self.__continuous_y).place(relx=self.COMMON_X - 0.1, rely=0.202)
		
		ctk.CTkLabel(self, text="Y Rotation:").place(relx=self.COMMON_X, rely=0.204, relheight=0.035, relwidth=0.075, anchor="ne")
		
		self.y_rotation_slider = ctk.CTkSlider(self, from_=-math.pi, to=math.pi, orientation="horizontal", command=self.__changed)
	...
	def __create_z_rot_slider(self):
		...
		ctk.CTkCheckBox(self, text="", variable=self._check_z_continuos, command=self.__continuous_z).place(
			relx=self.COMMON_X - 0.1, rely=0.283)
		
		ctk.CTkLabel(self, text="Z Rotation:").place(
			relx=self.COMMON_X, rely=0.285, relheight=0.035, relwidth=0.075, anchor="ne")
		
		self.z_rotation_slider = ctk.CTkSlider(self, from_=-math.pi, to=math.pi, orientation="horizontal", command=self.__changed)
		self.z_rotation_slider.set(0)
		self.z_rotation_slider.place(
			relx=self.COMMON_X, rely=0.325, relheight=0.04, relwidth=0.1, anchor="ne")
	...
	def __create_screenshot_button(self):
		ctk.CTkButton(self, text="Screenshot", command=self.__take_screenshot).place(relx=self.COMMON_X, rely=0.895, relheight=0.05, relwidth=0.1, anchor="ne")

	def __create_import_file_button(self):
		self._file_name = tk.StringVar()
		ctk.CTkLabel(self, textvariable=self._file_name).place(relx=0.01, rely=0.96, relheight=0.035, relwidth=0.4)
		ctk.CTkButton(self, text="Import file", command=self.__read_file).place(relx=self.COMMON_X, rely=0.815, relheight=0.05, relwidth=0.1, anchor="ne")
	...
	def __create_up_down_left_right_buttons(self):
		...
		ctk.CTkButton(self, text="U", command=self.__move_up).place(relx=COMM_X, rely=COMM_Y-0.04, relheight=COMM_RELATIVE_HIEGHT, relwidth=COMM_RELATIVE_WIDTH, anchor="ne")
		ctk.CTkButton(self, text="D", command=self.__move_down).place(relx=COMM_X, rely=COMM_Y+0.04, relheight=COMM_RELATIVE_HIEGHT, relwidth=COMM_RELATIVE_WIDTH, anchor="ne")
		ctk.CTkButton(self, text="L", command=self.__move_left).place(relx=COMM_X-0.035, rely=COMM_Y, relheight=COMM_RELATIVE_HIEGHT, relwidth=COMM_RELATIVE_WIDTH, anchor="ne")
		ctk.CTkButton(self, text="R", command=self.__move_right).place(relx=COMM_X+0.035, rely=COMM_Y, relheight=COMM_RELATIVE_HIEGHT, relwidth=COMM_RELATIVE_WIDTH, anchor="ne")
	...
	def __create_color_pickers(self):
		...
		ctk.CTkLabel(self, text="Fill color:").place(relx=0.01, rely=COMM_Y-0.01, relheight=0.035)
		self._fill_btn = ctk.CTkButton(self, text="", command=self.__pick_color_fill)
		...
		ctk.CTkLabel(self, text="Line color:").place(relx=0.2, rely=COMM_Y-0.01, relheight=0.035, anchor="ne")
		self._line_btn = ctk.CTkButton(self, text="", command=self.__pick_color_line)
		...
		self._canvas_btn = ctk.CTkButton(self, text="", command=self.__pick_color_canvas)
	...
	def __create_fill_check(self):
		...
		ctk.CTkCheckBox(self, text="No fill", variable=self._check_no_fill, command=self.__changed, onvalue=True, offvalue=False).place(relx=0.80, rely=0.92)
	...
	def __read_file(self):
		messagebox.showinfo(message='Only .obj and .brep files are compatible!', title="WARNING")

		file_path = filedialog.askopenfilename(defaultextension=".obj", filetypes=(("OBJ Files", "*.obj"), ("All Files", "*.*")))

		if file_path and not (file_path.endswith(".obj") or file_path.endswith(".brep")):
		...
		elif file_path.endswith(".brep"):
			self._file_name.set(file_path.split('/')[-1])
			self.__reset_rotation()
			with open(file_path) as file:
				self._geometry_handler.upload_object(*brep_files_handler.extract_data(file))
				self._file_exists = True
